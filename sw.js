const CACHE = 'sl-system-v6';
const ASSETS = [
  './',
  './index.html',
  './manifest.json',
  './videos/motivation1.mp4'
];
...
self.addEventListener('fetch', event => {
  if (event.request.url.endsWith('push-config.js')) {
    event.respondWith(
  ...
  // Video files — cache first, then network
      ...
      return fetch(event.request)
        .then(response => {
          if (
            ...
            response.status === 200 &&
            response.type === 'basic' &&
            !event.request.url.endsWith('push-config.js')
          ) {
            caches.open(CACHE).then(cache => cache.put(event.request, response.clone()));
          }
          return response;
